...
import net.snowflake.spark.snowflake.{Conversions, ProxyInfo, SnowflakeConnectorException, SnowflakeTelemetry}
  ...
  private[snowflake] def executorLogSystemConfigIfNotYet(): Unit = {
    if (!doesExecutorSystemConfigLogged) {
      ...
      logger.info(s"$WORKER_LOG_PREFIX: system config: " +
        s"${SnowflakeTelemetry.getSystemConfig().toPrettyString}")
    }
  }
}
